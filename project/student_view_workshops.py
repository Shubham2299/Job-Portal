...
t=cursor.fetchall()
def workshop_view(cid):
    session.setCid(cid)
    root.destroy()
    import view_workshop_s
def call():
    root.destroy()
    import student_home
    reload(student_home)    
try:
    ...
    for row in t:
        ...
        l6=Label(back,text=row[3],width=20,padx=10,pady=10,bd=5,bg='white')
        l6.grid(row=r,column=2)
        bta=Button(back,text='Show Info',width=10,bd=5,font='times 10')
        bta.config(command=lambda cid=row[12]:workshop_view(cid))
        bta.grid(row=r,column=3)
        r=r+1
except Exception as e:
    print('exception',e)
con.close()
bt1=Button(back,text='BACK TO HOME',font='times 11',width=18,bd=5,command=call)
bt1.grid(row=r,column=2)
