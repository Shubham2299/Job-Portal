...
con=pymysql.connect('localhost','root','root','project')
cursor=con.cursor()
y=session.getYear()
print(y)
cursor.execute("select p.u_id,s.name,p.date,c.name,c.package from placed_info as p ,student as s,company as c where p.date like '%/"+y+"' and p.u_id=s.uid and p.cid=c.company_id ")
t=cursor.fetchall()
def call():
    root.destroy()
    import tpo_search
